...
def DFS_bound(start, goal, depth, cycle_detection=False, verbose=False):
    frontier = []
    frontier.append([start])
    while frontier:
        path = frontier.pop() #select and remove last path from frontier
...
def DFS_bound2(start, goal, depth, cycle_detection, verbose=False):
    bases, _, patterns, columns = rank.get_information(start) #parameters for unrank
    frontier = []
    start_int = rank.rank(start) #convert start matrix to its rank integer
    frontier.append([start_int])
...
def DFS_deepening(start, goal, cycle_detection=False, verbose=False):
    max_length = 0
    frontier = []
    while True:
        if not frontier:
...
if __name__ == '__main__':
    ...
    path  = DFS_deepening(test_matrix, exit_row, True)
